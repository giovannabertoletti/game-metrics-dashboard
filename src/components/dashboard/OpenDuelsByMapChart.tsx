...
const OpenDuelsByMapChart = ({ data }: Props) => {
  ...
  return (
    <Card className="border-border/50 bg-card p-6">
      <h3 className="mb-1 font-display text-lg font-bold text-foreground">Ações por Mapa</h3>
      <p className="mb-6 text-xs text-muted-foreground">Ações positivas e negativas coletivas por mapa</p>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={data} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
          ...
          <Bar dataKey="positivos" name="Positivos" fill="hsl(155 100% 50%)" radius={[4, 4, 0, 0]} maxBarSize={32} />
          <Bar dataKey="negativos" name="Negativos" fill="hsl(0 70% 50%)" radius={[4, 4, 0, 0]} maxBarSize={32} />
        </BarChart>
      </ResponsiveContainer>
    </Card>
  );
